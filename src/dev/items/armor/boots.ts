...
Armor.registerOnTickListener(ItemID.infinity_boots, (item, slot, player) => {
    ...
    const swimming = AvaritiaFuncs.nativeIsActorInWater(player);
    if(flying || swimming) {
        ...
        if(vel.x > 0 || vel.y > 0 || vel.z > 0) {
            AvaritiaFuncs.nativeMoveActorRelative(player, 0, 0, 1, speed);
        } else if(vel.x < 0 || vel.y < 0 || vel.z < 0) {
            AvaritiaFuncs.nativeMoveActorRelative(player, 0, 0, 1, -speed * .3);
        }
        if(vel.x != 0 || vel.y != 0 || vel.z != 0) {
            AvaritiaFuncs.nativeMoveActorRelative(player, 1, 0, 0, speed * .5 * java.lang.Math.signum(vel.x != 0 ? vel.x : vel.y != 0 ? vel.y : vel.z));
        }
    }
});
