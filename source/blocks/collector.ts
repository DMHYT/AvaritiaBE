...
const GUI_COLLECTOR = new UI.Window({
    ...
    elements: (() => {
        ...
        const elems = {
            ...
            slotOutput: {type: "slot", x: 476.5, y: offset + 74, size: 47, isValid: () => false, visual: false},
...
class CollectorTileEntity extends TEImpl<CollectorTEDefaultValues> {
    ...
    public tick() {
        ...
        if(slot.id == 0 || (slot.id == ItemID.neutron_pile && slot.count < Item.getMaxStack(ItemID.neutron_pile))) {
            ...
            this.container.setText("textProgress", `Progress: ${(this.data.progress / COLLECTOR_PROCESS_TIME).toFixed(1)}%`);
            this.container.sendChanges();
        }
    }
