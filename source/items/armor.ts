...
Item.createArmorItem("infinity_leggings", "item.avaritia:infinity_boots.name", {name: "infinity_boots", meta: 0}, {type: "boots", armor: 6, durability: 9999, texture: "armor/infinity_0.png"}).setEnchantability(EEnchantType.BOOTS, 1000);
...
Armor.registerOnTickListener(ItemID.infinity_leggings, (item, slot, player) => {
    if(isEntityOnFire(player)) Entity.setFire(player, 0, true);
});
...
undestroyable_item(ItemID.infinity_helmet);
undestroyable_item(ItemID.infinity_chestplate);
undestroyable_item(ItemID.infinity_leggings);
undestroyable_item(ItemID.infinity_boots);